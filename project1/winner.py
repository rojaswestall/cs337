...
nlpStan = StanfordCoreNLP('http://localhost', port=9000, logging_level=logging.DEBUG, quiet=False)
...
def name(award_name, db_collection, collection_size):
  ...
  # tweets = db_collection.find({ 'timestamp_ms': { '$gt':interval[0], '$lt':interval[1] }, '$text': { '$search': 'winner win won congratulations' }} ).limit(200).sort('timestamp_ms', 1)
  ...
  top_entity = entities(clean_text, category, db_collection, collection_size)
...
def entities(tweets_text, category, db_collection, collection_size):
  arts = works_of_art(tweets_text, category)
  c = Counter(arts)
